...
func (app *application) userSignupPost(w http.ResponseWriter, r *http.Request) {
	...
	err := app.decodePostForm(r, &form)
	if err != nil {
		log.Println("DECODE ERROR:", err) // <-- Выведет причину в консоль
		app.clientError(w, http.StatusBadRequest)
		return
	}
...
func (app *application) accountPasswordUpdatePost(w http.ResponseWriter, r *http.Request) {
	...
	form.CheckField(validator.MatchesPassword(form.NewPasswordConfirmation, form.NewPassword), "newPasswordConfirmation", "Passwords do not match")
	if !form.Valid() {
		data := app.newTemplateData(r)
		data.Form = form
	...
	//stmt:=`select id form users where `
	id := app.sessionManager.GetInt(r.Context(), "authenticatedUserID")
